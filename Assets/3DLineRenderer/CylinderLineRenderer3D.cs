...
{
    [ExecuteAlways]
    class CylinderLineRenderer3D : MonoBehaviour
    {
        Mesh _mesh;
        ...
        void GenerateMesh()
        {
            center = transform.InverseTransformPoint(Vector3.Lerp(pointA.position, pointB.position, 0.5f));
            // Calculate the direction from Point A to Point B
            Vector3 direction = (pointB.position - pointA.position).normalized;

            // Calculate the rotation to align the Z-axis with the direction
            Quaternion rotation = Quaternion.FromToRotation(Vector3.up,direction);
            var height = Vector3.Distance(pointA.position, pointB.position);
            // Vertices
            ...
            for (int y = 0; y <= _heightSegments; y++)
            {
                ...
                for (int s = 0; s <= _segments; s++)
                {
                    ...
                    vertices[vertexIndex] = rotation * (center + new Vector3(x, currentHeight, z));
            ...
            vertices[^2] = rotation * (center + new Vector3(0, -height / 2, 0));
            ...
            //top cap triangles
            vertices[^1] = rotation * (center + new Vector3(0, height / 2, 0));
